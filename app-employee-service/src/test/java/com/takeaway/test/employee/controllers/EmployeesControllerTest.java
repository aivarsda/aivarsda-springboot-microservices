...
import com.aivarsd.jwtauth.Application;
...
import com.takeaway.test.employee.UserRole;
...
//        Department department = departmentDao.create(Department.builder().name("dept_name").build());
        ...
        rolesSet.add(UserRole.ROLE_USER.toString());
...
//        request.setBirthDay(LocalDate.of(2012, 1, 1));
        request.setRole(rolesSet);
        request.setPassword("testpass123");
        MvcResult result = mvc.perform(post(URI + "/signup").contentType(MediaType.APPLICATION_JSON).content(mapToJson(request))).andReturn();
